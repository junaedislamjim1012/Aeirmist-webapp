...
export const MediaEditor: React.FC<MediaEditorProps> = ({ mediaUrl, mediaType, onClose, onSave }) => {
  ...
  const drawSticker = (ctx: CanvasRenderingContext2D, canvasWidth: number, canvasHeight: number) => {
    ...
    ctx.fillText(`Anonymous Message #${nodeId}`.toUpperCase(), footerX, footerY + footerFontSize + cardWidth * 0.04);
    ctx.restore();
  };
  ...
  const handleSave = () => {
      ...
      canvas.toBlob(
        (blob) => {
          ...
          if (blob) {
            ...
            logger.error("Failed to generate composite blob from canvas, falling back.");
            // Fallback: fetch original mediaUrl directly
            fetch(mediaUrl)
              .then(res => res.blob())
              .then(onSave)
              .catch(err => {
                logger.error("Save fallback failed:", err);
                onSave(new Blob());
              });
          }
        },
    ...
    if (mediaType === 'image') {
      ...
      img.onerror = (err) => {
        logger.error("Failed to load image on canvas, using dimensions:", err);
        canvas.width = 1080;
        canvas.height = 1920;
        ctx.fillStyle = '#000000';
      ...
      video.onerror = (err) => {
        logger.error("Failed to load video seek on canvas:", err);
        canvas.width = 1080;
        canvas.height = 1920;
        ctx.fillStyle = '#000000';
        ctx.fillRect(0, 0, canvas.width, canvas.height);
