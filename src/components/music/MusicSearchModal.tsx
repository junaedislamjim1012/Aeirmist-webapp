...
export const MusicSearchModal = ({ onClose, onSelect }: { onClose: () => void, onSelect: (song: any) => void }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      logger.error('Error subscription to sound_library:', error);
      setLibraryLoading(false);
    });
  ...
  const handlePlayPause = (e: React.MouseEvent, track: any) => {
    ...
    if (playingTrackId === track.id) {
      ...
      audioRef.current.play().catch(err => {
        logger.error('Audio preview error:', err);
        addToast?.({
              ...
              {!spotifyLoading && spotifyResults.length === 0 && (
                <div className="text-center py-12 px-6">
                  ...
                  <p className="text-xs text-white/40">
                    {spotifyQuery.trim() ? 'No Spotify tracks found matching your search.' : 'Enter a track or artist to search the Spotify catalog.'}
                  </p>
                </div>
              )}
            </div>
