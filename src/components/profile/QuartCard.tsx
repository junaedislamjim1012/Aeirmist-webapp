...
export const QuartCard: React.FC<QuartCardProps> = ({ post, onUserClick, onCommentClick }) => {
  ...
  const handleRepost = async () => {
    if (addToast) {
      addToast({
        title: 'SHARED',
        message: 'This post has been shared to your Feed.',
        type: 'success'
      });
    }
    ...
    if (navigator.share) {
      try {
        ...
        if (e?.name !== 'AbortError') {
          logger.warn("Share aborted, copying link:", e);
        } else {
          return;
        }
      }
    ...
    try {
      ...
      if (addToast) {
        addToast({
          ...
          message: post.isArchived ? 'Post restored to active feed.' : 'Post archived.',
          type: 'success'
        });
      }
    } catch (e) {
