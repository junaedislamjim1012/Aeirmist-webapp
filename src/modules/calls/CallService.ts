...
class CallService {
  ...
  private setupPeerConnection(db: any, iceServers: RTCIceServer[], onRemoteStream: (stream: MediaStream) => void) {
    ...
    this.peerConnection.ontrack = (event) => {
      logger.info("[WebRTC] Remote track received", event.streams[0]);
    ...
    this.peerConnection.onconnectionstatechange = () => {
      logger.info("[WebRTC] Connection state:", this.peerConnection?.connectionState);
      const state = this.peerConnection?.connectionState;
      if (state === 'connected' && this.callId && !this.isSafeMode) {
        updateDoc(doc(db, 'calls', this.callId), { status: 'ongoing' }).catch(() => {});
      } else if (state === 'failed' || state === 'disconnected') {
        logger.warn("[WebRTC] Connection unstable or failed:", state);
        if (state === 'failed') {
          this.cleanup();
        } else {
    ...
    const handleNetworkChange = () => {
      if (!navigator.onLine) {
        logger.warn("[WebRTC] Network went offline");
        if (this.callId && db) {
    ...
    this.peerConnection.oniceconnectionstatechange = () => {
       logger.info("[WebRTC] ICE Connection state:", this.peerConnection?.iceConnectionState);
    };
  }
  ...
  private async flushCandidates(db: any) {
    ...
    try {
      ...
      logger.warn("Failed to flush candidates", e);
    }
  }
